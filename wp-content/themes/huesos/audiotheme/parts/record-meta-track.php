...
?>
	...
	<figure class="record-artwork">
		<a class="post-thumbnail" href="<?php echo get_permalink( $post->post_parent ); ?>" itemprop="image">
...
?>
	...
	<div class="record-details record-details--track">
		<h2 class="screen-reader-text"><?php _e( 'Record Details', 'huesos' ); ?></h2>
		<dl>
			<?php if ( $artist ) : ?>
				<dt class="record-artist screen-reader-text"><?php _e( 'Artist', 'huesos' ); ?></dt>
				<dd class="record-artist" itemprop="byArtist"><?php echo esc_html( $artist ); ?></dd>
			<?php endif; ?>

			<?php if ( $year ) : ?>
				<dt class="record-year"><?php _e( 'Release', 'huesos' ); ?></dt>
				<dd class="record-year" itemprop="dateCreated"><?php echo esc_html( $year ); ?></dd>
			<?php endif; ?>

			<?php if ( $genre ) : ?>
				<dt class="record-genre"><?php _e( 'Genre', 'huesos' ); ?></dt>
...
?>
	...
	<div class="meta-links">
		<h2 class="screen-reader-text"><?php _e( 'Track Links', 'huesos' ); ?></h2>
		<ul>
			<?php if ( $purchase_url ) : ?>
				<li><a class="button" href="<?php echo esc_url( $purchase_url ); ?>" itemprop="url"><?php _e( 'Purchase', 'huesos' ); ?></a></li>
			<?php endif; ?>

			<?php if ( $download_url ) : ?>
				<li><a class="button" href="<?php echo esc_url( $download_url ); ?>" itemprop="url" download="<?php esc_attr( basename( $download_url ) ); ?>"><?php _e( 'Download', 'huesos' ); ?></a></li>
			<?php endif; ?>
		</ul>
	</div>
