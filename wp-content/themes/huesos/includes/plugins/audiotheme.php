...
function huesos_audiotheme_sidebar_status( $is_active_sidebar, $index ) {
	...
	if ( is_audiotheme_post_type_archive() ) {
		$is_active_sidebar = ( 'enabled' === get_audiotheme_archive_meta( 'huesos_sidebar', true, 'enabled' ) );
	} elseif ( is_singular( array( 'audiotheme_gig', 'audiotheme_record', 'audiotheme_track', 'audiotheme_video' ) ) ) {
		$post_type         = 'audiotheme_track' == get_post_type() ? 'audiotheme_record' : get_post_type();
		$is_active_sidebar = ( 'enabled' === get_audiotheme_archive_meta( 'huesos_sidebar_singular', true, 'enabled', $post_type ) );
	}
...
function huesos_audiotheme_record_archive_grid_meta( $post_id ) {
	...
	printf(
		...
		get_audiotheme_record_artist( $post_id )
	);
}
...
function huesos_audiotheme_admin_save_archive( $post_id, $post, $post_type ) {
	...
	$value = isset( $_POST['huesos_featured_video'] ) ? $_POST['huesos_featured_video'] : '';
...
function huesos_audiotheme_admin_archive_meta_box( $post ) {
	...
	<p>
		<label for="huesos-sidebar-status">
			...
			<?php _e( 'Disable the sidebar on this archive?', 'huesos' ); ?>
		</label>
	</p>
	<p>
		<label for="huesos-sidebar-singular-status">
			...
			<?php _e( 'Disable the sidebar for posts in this archive?', 'huesos' ); ?>
		</label>
	</p>
	<?php
}
...
function huesos_audiotheme_admin_video_archive_meta_box( $post ) {
	...
	?>
	<p>
		<label for="huesos-featured-video"><?php _e( 'Featured Video:', 'huesos' ); ?></label>
...
function huesos_ajax_audiotheme_get_video_data() {
	wp_send_json( array(
		'videoHtml' => get_audiotheme_video( $_GET['post_id'] ),
	) );
