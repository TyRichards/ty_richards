...
class Cedaro_Theme_FeaturedContent {
	...
	public function customize_register( $wp_customize ) {
		$wp_customize->add_section( 'featured_content', array(
			'title'       => __( 'Featured Content', 'huesos' ),
			'description' => sprintf( __( 'Your theme supports up to %d posts in its featured content area.', 'huesos' ), $this->max_posts ),
			'priority'    => 130,
		) );
	...
	public function print_templates() {
		...
		<script type="text/html" id="tmpl-cedaro-theme-featured-content-button-add">
			<button class="button button-secondary alignright"><?php _e( 'Add Posts', 'huesos' ); ?></button>
		</script>
		...
		<script type="text/html" id="tmpl-cedaro-theme-featured-content-post">
			...
			<div class="ctfc-post-inside">
				<div class="ctfc-post-actions">
					<a class="ctfc-post-remove js-remove"><?php _e( 'Delete', 'huesos' ); ?></a> |
					<a class="js-close"><?php _e( 'Close', 'cue', 'huesos' ); ?></a>
				</div>
			</div>
		</script>
		...
		<script type="text/html" id="tmpl-cedaro-theme-featured-content-modal">
			...
			<div class="ctfc-modal-inside find-box-inside">
				<div class="ctfc-modal-search find-box-search">
					...
					<input type="button" value="<?php esc_attr_e( 'Search', 'huesos' ); ?>" class="button ctfc-modal-search-button" />
					<div class="clear"></div>
				</div>
	...
	public function ajax_find_posts() {
		...
		if ( ! $posts ) {
			wp_send_json_error( __( 'No items found.', 'huesos' ) );
		}
	...
	protected function get_found_posts_html( $posts ) {
		...
		foreach ( $posts as $post ) {
			$title = trim( $post->post_title ) ? $post->post_title : __( '(no title)', 'huesos' );
			$alt   = ( 'alternate' == $alt ) ? '' : 'alternate';

			switch ( $post->post_status ) {
				case 'publish' :
				case 'private' :
					$stat = __( 'Published', 'huesos' );
					break;
				case 'future' :
					$stat = __( 'Scheduled', 'huesos' );
					break;
				case 'pending' :
					$stat = __( 'Pending Review', 'huesos' );
					break;
				case 'draft' :
					$stat = __( 'Draft', 'huesos' );
					break;
			}
			...
			if ( '0000-00-00 00:00:00' == $post->post_date ) {
				...
				$time = mysql2date( __( 'Y/m/d', 'huesos' ), $post->post_date );
			}
